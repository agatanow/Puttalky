...
public class BazaWiedzy {
	...
	private Set<OWLClass> listaWege;
	private Set<OWLClass> listaMieso;
    OWLReasoner silnik;
    ...
    public void inicjalizuj() {
		...
		try {
			...
			listaWege = new HashSet<OWLClass>();
			listaMieso = new HashSet<OWLClass>();
			...
			OWLClass pizzaWege  = manager.getOWLDataFactory().getOWLClass(IRI.create("http://semantic.cs.put.poznan.pl/ontologie/pizza.owl#PizzaWegetariańska"));
			for (org.semanticweb.owlapi.reasoner.Node<OWLClass> klasa: silnik.getSubClasses(pizzaWege, false)) {
				listaWege.add(klasa.getRepresentativeElement());
			}

			OWLClass pizzaMieso  = manager.getOWLDataFactory().getOWLClass(IRI.create("http://semantic.cs.put.poznan.pl/ontologie/pizza.owl#PizzaZMięsem"));
			for (org.semanticweb.owlapi.reasoner.Node<OWLClass> klasa: silnik.getSubClasses(pizzaMieso, false)) {
				listaMieso.add(klasa.getRepresentativeElement());
			}
	...
	public Set<String> dopasujPizzePoTypie(String s){
		Set<String> result = new HashSet<String>();
		if(s.equals("PizzaWegetariańska")) {
			for (OWLClass klasa : listaWege) {
				result.add(klasa.getIRI().getFragment());

			}
		}
		if(s.equals("PizzaZMięsem")) {

			for (OWLClass klasa : listaMieso) {
				result.add(klasa.getIRI().getFragment());
	...
	public Set<String> wyszukajPizzePoDodatkach(Set<String> zbior){
